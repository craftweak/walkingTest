...
#if defined(V_WIRE_REFLECTION_ON) && defined(V_WIRE_VERTEX_COLOR_ON) && defined(_NORMALMAP) 
...
struct vOutput
{
	...
	half4 uv : TEXCOORD0;	//xy - mainTex, zw - wireTransparencyTex
...
//V_CW_TransformPointAndNormal(v.vertex, v.normal, v.tangent);
	...
	UNITY_TRANSFER_FOG(o, o.pos);
		...
		o.lm = v.texcoord1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
			...
			#ifdef V_WIRE_VERTEX_LIGHT_AND_AMBIENT_ON
...
{			
	...
	half dynamicMask = 1;
		...
		#ifndef V_WIRE_VERTEX_LIGHT_AND_AMBIENT_ON
	...
	//Wire
			...
			#ifdef V_WIRE_TRANSPARENCY_ON
				half customAlpha = tex2D(_V_WIRE_TransparentTex, i.uv.zw).a;
			...
			DoWire(retColor, i.mass, dynamicMask);
		#endif
	#else
