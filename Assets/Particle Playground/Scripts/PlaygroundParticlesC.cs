...
namespace ParticlePlayground {
	...
	public class PlaygroundParticlesC : MonoBehaviour {
		...
		*************************************************************************************************************************************************/
		...
		[HideInInspector] public float localTime = 0f;
		...
		*************************************************************************************************************************************************/
		...
		public void UpdateShuriken () {
			...
			if (!isReadyForThreadedCalculations || !calculate) return;

			// Sync positions to main-thread
			if (syncPositionsOnMainThread || (forceSkinnedMeshUpdateOnMainThread && source==SOURCEC.SkinnedWorldObject && (onlySourcePositioning||onlyLifetimePositioning))) {
		...
		Color32 GetParticleColor (int p, float normalizedLife, float normalizedP) {
			...
			if (colorMethod==COLORMETHOD.ParticleArray && arrayColorUsesAlpha) {
				...
				currentColor.a = arrayAlphaColor.a;
			}
			
			// Assign mask alpha
			if (applyParticleMask) {
				currentColor.a*=playgroundCache.maskAlpha[p];
			}
		...
		/// </summary>
		private bool isDoneThread = true;
	...
	public class PlaygroundCache {
		...
		/// <summary>
		/// Determines if the particle is calculated during this frame. 
